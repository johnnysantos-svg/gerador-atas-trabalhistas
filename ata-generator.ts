...
import { REPLICA_TEXTS, CONTESTACAO_TEXTS } from './constants';
...
export const generateAtaHtml = (data: AtaData): string => {
    ...
    // 8. Atos Processuais
    switch(data.atosProcessuaisOpcao) {
        case AtosProcessuaisOpcao.INICIAL:
            html += formatParagraphs(`Audiência de <strong>INSTRUÇÃO TELEPRESENCIAL</strong> para o dia ${data.instrucaoData || '[DATA]'} às ${data.instrucaoHora || '[HORA]'}, quando as partes deverão comparecer, na forma da lei, para prestar depoimento pessoal, sob pena de confissão ficta, sendo desde logo informado que as testemunhas comparecerão independentemente de intimação, na forma do artigo 825 da CLT.`);
            break;
        case AtosProcessuaisOpcao.PERICIA_MEDICA:
            {
                const text = `Em razão da alegação de ${data.periciaMedicaDoenca || '[TIPO DE DOENÇA]'}, o Juiz determinou a realização de perícia, a cargo do(a) Dr(a). ${data.periciaMedicaPerito || '[NOME DO PERITO]'}, que fica desde já nomeado(a) e que deverá ser notificado(a) para entregar o laudo no prazo de 30 dias. O(A) SR.(A) PERITO(A) DEVERÁ COMUNICAR ÀS PARTES, POR ESCRITO, A DATA E HORÁRIO DA REALIZAÇÃO DA PERÍCIA.
...
<strong>AUTOS FORA DE PAUTA. AGUARDAR CONCLUSÃO DA PERÍCIA. AS PARTES E SEUS ADVOGADOS SERÃO INTIMADOS DA PRÓXIMA AUDIÊNCIA.</strong>`;
        ...
        case AtosProcessuaisOpcao.PERICIA_CONTABIL:
             {
                const text = `Determina-se realização de perícia contábil, nomeando-se de logo ${data.periciaContabilPerito || '[NOME DO PERITO]'}, que deverá apresentar o laudo em 30 dias.
...
As partes e patronos são orientados, neste momento, de que as perguntas serão realizadas por "tópicos" e na ordem, acordados previamente entre as partes e o Juízo. Esta sistemática tem como intuito o cumprimento do art. 3º da Resolução n. 313/2021 do CSJT.`;
                ...
                if (data.livreTexto) {
                    gravacaoHtml += formatParagraphs(data.livreTexto);
                }
                ...
                gravacaoHtml += formatParagraphs(`Cientes os presentes.`);
                gravacaoHtml += formatParagraphs(`Audiência encerrada às ${data.encerramentoHora || 'XX:XX'}.`);
                gravacaoHtml += formatParagraphs(`Nada mais.`);
        ...
        case AtosProcessuaisOpcao.MATERIA_DIREITO:
            {
                const text = `Pela ordem, as partes não têm provas pessoais a apresentar, por se tratar de matéria de direito e de fato calcada em prova documental, em razão do que o Juízo determinou o julgamento antecipado da lide.
...
Autos conclusos para sentença, da qual os procuradores serão intimados.`;
        ...
        case AtosProcessuaisOpcao.ADIAMENTO_FRACIONAMENTO:
            {
                const text = `Pelo Juízo, tendo em vista a ${data.adiamentoMotivo || '[MOTIVO]'}, determinou-se o adiamento/fracionamento desta assentada para a data abaixo designada.
...
AUTOS SOBRESTADOS EM RAZÃO DA DECISÃO DO C.STF`;
    ...
    const hasMainAtosContent = !!data.atosProcessuaisOpcao && data.atosProcessuaisOpcao !== AtosProcessuaisOpcao.LIVRE;
    if (data.livreTexto) {
        if (hasMainAtosContent) {
            html += '<br/>';
        }
...
const createParagraphsFromText = (text: string): Paragraph[] => {
  return text.split('\n\n').map(pText => {
    ...
    const segments = pText.split(/(<strong>.*?<\/strong>)/g).filter(Boolean);
    ...
    return new Paragraph({ children, style: "normal" });
  });
};


export const generateDocx = (data: AtaData): Document => {
  // FIX: Moved paragraph array initialization to the top and document creation to the bottom.
  // This resolves the error from trying to access `doc.sections` which is not available on a Document instance.
  ...
  Array.from(tempDiv.children).forEach(child => {
      ...
      if (child.tagName === 'H2') {
          ...
          // FIX: Removed buggy logic that attempted to mutate readonly properties of TextRun,
          // which caused the 'options does not exist' error. The style inheritance logic above makes this redundant.
